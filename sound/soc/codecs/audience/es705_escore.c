				       ...
				       char *buf)
{
	...
	value = escore_read(NULL, ES705_CHANGE_STATUS);
					...
					const char *buf, size_t count)
{
	...
	es705->mode = SBL;
	rc = escore_write(NULL, ES705_POWER_STATE , value);
	if (rc) {
				...
				const char *buf, size_t count)
{
	...
	rc = escore_write(NULL, ES705_EVENT_RESPONSE, value);
	if (rc)
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	/* 0 = NS off, 1 = NS on*/
	if (value)
		rc = escore_write(NULL, ES705_PRESET,
			ES705_NS_ON_PRESET);
	else
		rc = escore_write(NULL, ES705_PRESET,
			ES705_NS_OFF_PRESET);
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (value == ES705_AUD_ZOOM_NARRATOR) {
		...
		rc = escore_write(NULL, ES705_PRESET, 0);
...
int es705_core_probe(struct device *dev)
{
	...
	const char *bkg_filename = "audience/adnc_cvq_detection_bkg_w_hdrs.bin";
#else
