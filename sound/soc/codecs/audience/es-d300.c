...
static struct switch_setting es_d300_switch_pt_vp[SW_MAX] = {
	[SWIN1] = {
		.value_cmd[0] = 0xB0660009, /* Input 0 */
		.value_cmd[1] = 0xB0660109, /* Input 1 */
		.value = 0, /* Default Value */
	},
	[SWIN2] = {
		.value_cmd[0] = 0xB066000A, /* Input 0 */
		.value_cmd[1] = 0xB066010A, /* Input 1 */
		.value = 0, /* Default Value */
	},
	[SWOUT0] = {
		.value_cmd[0] = 0xB0660000, /* Output 0 */
		.value_cmd[1] = 0xB0660100, /* Output 1 */
		.value = 0, /* Default Value */
	},
	[SWOUT1] = {
		.value_cmd[0] = 0xB0660001, /* Output 0 */
		.value_cmd[1] = 0xB0660101, /* Output 1 */
		.value = 0, /* Default Value */
	},
	[SWOUT2] = {
		.value_cmd[0] = 0xB0660002, /* Output 0 */
		.value_cmd[1] = 0xB0660102, /* Output 1 */
		.value = 0, /* Default Value */
	},
	[SWOUT3] = {
		.value_cmd[0] = 0xB0660003, /* Output 0 */
		.value_cmd[1] = 0xB0660103, /* Output 1 */
		.value = 0, /* Default Value */
	},
};
static struct switch_setting es_d300_switch[ALGO_MAX][SW_MAX];
...
static int escore_set_switch(int id)
{
	...
	switch_value = es_d300_switch[escore->algo_type][id].value;
	cmd = es_d300_switch[escore->algo_type][id].value_cmd[switch_value];
...
static int es300_codec_stop_algo(struct escore_priv *escore)
{
	...
	if (ret)
		pr_err("%s: algo stop failed, ret = %d\n", __func__, ret);
...
static int es_set_digital_gain(struct escore_priv *escore)
{
	int rc   = 0, i;
	u32 cmd  = 0;
	u32 resp = 0;
	...
	if (escore->digital_gain_num > 0) {
		for (i = 0; i < escore->digital_gain_num; i++) {
			pr_debug("%s: Select path ID %d and set digital gain 0x%x.\n",
				 __func__,
				 escore->digital_gain[i].path_id,
				 escore->digital_gain[i].digital_gain);

			/* Select path ID */
			cmd = (ES_SELECT_ENDPOINT<<16) | escore->digital_gain[i].path_id;
...
static int es_set_final_route(struct escore_priv *escore)
{
	...
	if (escore->algo_type == PASSTHRU_VP) {
		...
		case STEREO_CAPTURE:
			if (pri && fein) {
				/* Stereo capture */
				es_d300_switch[escore->algo_type][SWIN1].value
					= 0;
			} else if (pri) {
		...
		for (i = 0; i < SW_MAX; i++) {
			if (!es_d300_switch[escore->algo_type][i].value)
				continue;

			rc = escore_set_switch(i);
			if (rc) {
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	/* Only PT_VP algo is supported */
	if (escore_priv.algo_type != PASSTHRU_VP) {
		pr_err("%s(): Only PT_VP is supported\n", __func__);
		rc = -EINVAL;
		goto exit;
	}

	escore_priv.pri_pb_mode = ucontrol->value.enumerated.item[0];

	/* Identify usecase to configure switch settings */
	switch (escore_priv.pri_pb_mode) {
	/* Off */
	case PRI_PB_OFF:
		es_d300_switch[escore_priv.algo_type][SWOUT0].value = 0;
		es_d300_switch[escore_priv.algo_type][SWOUT1].value = 0;
		break;
	/* Stereo */
	case PRI_PB_STEREO:
		es_d300_switch[escore_priv.algo_type][SWOUT0].value = 1;
		es_d300_switch[escore_priv.algo_type][SWOUT1].value = 1;
		break;
	/* Invalid */
	default:
	...
	if (atomic_read(&escore_priv.active_streams)) {
		rc = escore_set_switch(SWOUT0);
		if (rc)
			goto exit;
		rc = escore_set_switch(SWOUT1);
	}
exit:
	return rc;
}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	escore_priv.sec_pb_mode = ucontrol->value.enumerated.item[0];

	/* Identify usecase to configure switch settings */
	switch (escore_priv.sec_pb_mode) {
	/* Off */
	case SEC_PB_OFF:
		es_d300_switch[escore_priv.algo_type][SWIN2].value = 0;
		es_d300_switch[escore_priv.algo_type][SWOUT2].value = 0;
		es_d300_switch[escore_priv.algo_type][SWOUT3].value = 0;
		break;
	/* Mixed Mono */
	case SEC_PB_MIXED_MONO:
		es_d300_switch[escore_priv.algo_type][SWIN2].value = 0;
		es_d300_switch[escore_priv.algo_type][SWOUT2].value = 0;
		es_d300_switch[escore_priv.algo_type][SWOUT3].value = 1;
		break;
	/* Stereo */
	case SEC_PB_STEREO:
		es_d300_switch[escore_priv.algo_type][SWIN2].value = 1;
		es_d300_switch[escore_priv.algo_type][SWOUT2].value = 1;
		es_d300_switch[escore_priv.algo_type][SWOUT3].value = 1;
		break;
	/* Invalid */
	default:
	...
	/* Set the switches for if route is already active */
	if (atomic_read(&escore_priv.active_streams)) {
		rc = escore_set_switch(SWIN2);
		if (rc)
			goto exit;
		rc = escore_set_switch(SWOUT2);
		if (rc)
			goto exit;
		rc = escore_set_switch(SWOUT3);
	}
exit:
	return rc;
}
...
int es_d300_fill_cmdcache(struct snd_soc_codec *codec)
{
	...
	/* Switch configuration for PT_VP algo */
	memcpy(es_d300_switch[PASSTHRU_VP], es_d300_switch_pt_vp,
		sizeof(es_d300_switch[PASSTHRU_VP]));
