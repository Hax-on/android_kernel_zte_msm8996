...
static struct clk *spi6_iface_clk;
static struct clk *spi6_icore_clk;
...
static int __init fpc1020_init(void)
{
	...
	if (spi_register_driver(&fpc1020_driver))
		return -EINVAL;
	printk("FP_DEBUG_MXP: fpc1020_init is successful.\n");
	return 0;
}
...
static int fpc1020_probe(struct spi_device *spi)
{
	...
	printk("FP_DEBUG_MXP2015_0828_3: Enter fpc1020_probe...\n");
...
#if 0
	fpc1020_gpio_reset(fpc1020); //only test by stone 20150826

	return 0; //only test by stone 20150824
#endif
...
static int fpc1020_spi_clk_ctl(fpc1020_data_t *fpc1020, bool onoff)
{
	int ret = 0;
  	...
  	if(spi6_iface_clk&&spi6_icore_clk)
  	{
		if(onoff)
		{
			if(!spi_clk_enable)
			{
				spi_clk_enable = true;
				printk("%s: turn on spi6_iface_clk\n", __func__);
				ret = clk_prepare_enable(spi6_iface_clk);
				if(ret){
		    			printk(KERN_ERR "%s: prepare spi6_iface_clk failed ret:%d\n", __func__, ret);
				}

				printk("%s: turn on spi6_icore_clk\n", __func__);
				ret = clk_prepare_enable(spi6_icore_clk);
				if(ret){
		    			printk(KERN_ERR "%s: prepare spi6_icore_clk failed ret:%d\n", __func__, ret);
				}
			}

			else
			{
                      printk("%s: turn on spi6_iface_clk,is ready\n", __func__);
			}
		}
		else
		{
			if(spi_clk_enable)
			{
				...
				printk("%s: turn off spi6_iface_clk\n", __func__);
	//			clk_disable_unprepare(spi6_iface_clk);
				printk("%s: turn off spi6_icore_clk\n", __func__);
	//			clk_disable_unprepare(spi6_icore_clk);
			}
			else
			{
                      printk("%s: turn on spi6_iface_clk,is off\n", __func__);
			}
		}
	}
	else
					...
					struct fpc1020_platform_data *pdata)
{
	...
	pdata->vcc_en_gpio = of_get_named_gpio(node, "vcc-en-gpio", 0);
      ...
      zte_gc=gpio_to_chip(pdata->irq_gpio); //only test

	printk("FP_DEBUG_MXP: %s : irq_gpio = %d, reset_gpio = %d, cs_gpio = %d, vcc_en_gpio = %d\n", __func__, 
		pdata->irq_gpio-zte_gc->base, pdata->reset_gpio-zte_gc->base, pdata->cs_gpio-zte_gc->base, pdata->vcc_en_gpio-zte_gc->base);
	...
	spi6_iface_clk = devm_clk_get(dev,"spi6_iface_clk");
	if (IS_ERR(spi6_iface_clk)) {
		printk(KERN_ERR "%s: Error getting spi6_iface_clk\n", __func__);
		spi6_iface_clk = NULL;
	}
	else
	{
		printk("%s: spi6_iface_clk get OK!\n", __func__);
	}
	
	spi6_icore_clk = devm_clk_get(dev,"spi6_icore_clk");
	if (IS_ERR(spi6_icore_clk)) {
		printk(KERN_ERR "%s: Error getting spi6_icore_clk\n", __func__);
		spi6_icore_clk = NULL;
	}
	else
	{
		printk("%s: spi6_icore_clk get OK!\n", __func__);
	}
