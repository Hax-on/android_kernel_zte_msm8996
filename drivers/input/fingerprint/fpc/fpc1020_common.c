...
int fpc1020_setup_defaults(fpc1020_data_t *fpc1020)
{
	...
	printk("FP_DEBUG_MXP: fpc1020_setup_defaults: fpc1020->chip.type = %d.\n",fpc1020->chip.type);
	...
	printk("FP_DEBUG_MXP: fpc1020_setup_defaults: OK.\n");
	return 0;
...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	while (counter) {
		...
		irq_gpio_value = gpio_get_value(fpc1020->irq_gpio);//
		printk("FP_DEBUG_MXP: After reset irq_gpio_value = %d.\n",irq_gpio_value);//
...
int fpc1020_check_hw_id(fpc1020_data_t *fpc1020)
{
	...
	printk("FP_DEBUG_MXP: fpc1020_check_hw_id: fpc1020->force_hwid = %d.\n",fpc1020->force_hwid);
...
int fpc1020_wake_up(fpc1020_data_t *fpc1020)
{
	...
	int status = fpc1020_read_status_reg(fpc1020);
 	//printk("FP_DEBUG_MXP: Enter fpc1020_wake_up,reset=%d,status=%d.\n",reset,status);
	if (reset == 0 && status >= 0 &&
		(fpc1020_status_reg_t)(status & status_mask) ==
		FPC1020_STATUS_REG_IN_IDLE_MODE) {

		//dev_dbg(&fpc1020->spi->dev, "%s OK\n", __func__);
		//printk("FP_DEBUG_MXP: fpc1020_wake_up,OK.\n");
		return 0;
	} else {

		//dev_err(&fpc1020->spi->dev, "%s FAILED\n", __func__);
		//printk("FP_DEBUG_MXP: fpc1020_wake_up,FAILED.\n");
		return -EIO;
	}
}
