...
#include <linux/ioctl.h> // for TEE IRQ
#include <linux/sched.h>
#include <linux/wakelock.h>//maxiaoping 20140108
...
typedef struct {
	...
	struct pinctrl_state *fpc_cs_active;
	struct pinctrl_state *fpc_cs_suspend;
...
//add by maxiaoping 20150326
#define FPC_POWER_ON			_IOW('K', 6, int)
#define FPC_POWER_DOWN		_IOW('K', 7, int)
#define FPC_CLOCK_ON			_IOW('K', 8, int)
#define FPC_CLOCK_OFF			_IOW('K', 9, int)
#define FPC_UEVENT_ENABLE		_IOW('K', 10, int)
