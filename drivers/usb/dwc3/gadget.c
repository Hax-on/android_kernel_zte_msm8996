...
int scsicmd_start_adbd(void)
{
	...
	printk(KERN_ERR"usb_xbl: %s, %d  %d\n",__FUNCTION__, __LINE__, dwc->start_adbd);
...
static void dwc3_gadget_wakeup_work(struct work_struct *w)
{
	...
	pr_info("dwc3 pm usage count " "Gdgwake gsyn " "%d\n",
		atomic_read(&dwc->dev->power.usage_count));
	...
	dbg_event(0xFF, "Gdgwake put",
		atomic_read(&dwc->dev->power.usage_count));
	pr_info("dwc3 pm usage count " "Gdgwake put " "%d\n",
		atomic_read(&dwc->dev->power.usage_count));
}
...
static int dwc3_gadget_pullup(struct usb_gadget *g, int is_on)
{
	...
	dbg_event(0xFF, "Pullup gsync",
		atomic_read(&dwc->dev->power.usage_count));
	pr_info("dwc3 pm usage count " "Pullup gsync " "%d\n",
		atomic_read(&dwc->dev->power.usage_count));
	...
	dbg_event(0xFF, "Pullup put",
		atomic_read(&dwc->dev->power.usage_count));
	pr_info("dwc3 pm usage count " "Pullup put " "%d\n",
		atomic_read(&dwc->dev->power.usage_count));
...
static int dwc3_gadget_vbus_session(struct usb_gadget *_gadget, int is_active)
{
	...
	pr_info("usb %s, activate:%d, softconnect:%d\n",__func__, is_active,dwc->softconnect);
	return 0;
}
		...
		unsigned int evtinfo)
{
	...
	dev_info(dwc->dev, "Going from (%d)--->(%d)\n", dwc->link_state, next);
	dwc->link_state = next;
	wake_up_interruptible(&dwc->wait_linkstate);
}
...
int dwc3_gadget_init(struct dwc3 *dwc)
{
	...
	if (ret) {
		printk("%s register switch event for on/offline error,errno = %d\n", __func__, ret);
		goto err5;
	}
