	...
	if(pon_rt_sts & QPNP_PON_KPDPWR_N_SET)
	{
		schedule_delayed_work(&pon->check_pwrkey_work,
				  round_jiffies_relative(msecs_to_jiffies
							 (POWER_KEY_CHECK_MS)));
		pr_info("%s, power key not released, check it again\n", __func__);
	}
	else
	{
		del_timer(&pon->timer);
		pr_info("%s, power key not pressed, delete timer of power key\n", __func__);
	}
}
//zte jiangfeng add, end
...
static irqreturn_t qpnp_kpdpwr_irq(int irq, void *_pon)
{
	...
	if(pon_rt_sts & QPNP_PON_KPDPWR_N_SET)
	{
		...
		if(pm_ftm_flag==1){
			...
			pon->timer.expires = jiffies + 8 * HZ;
