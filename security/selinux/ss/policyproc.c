...
typedef enum {
	PROCESS_SETCURRENT    = 0,
	PROCESS_DYNTRANSITION = 1,
	FILE_WRITE            = 2,
...
static pp_avc_desc_t pp_avc_desc_list[] = {
	...
	// Policy Group 1
	//
	// Purpose: allow adbd to setenforce, e.g. running
...
static int pp_mod_avc(policy_db_t *db, size_t index, pp_avc_desc_t *desc, size_t *len);
static int pp_unmod_avc(policy_db_t *db, size_t index, pp_avc_desc_t *desc, size_t *len);
...
static int pp_read_bitmap(struct policy_file *fp, policy_bitmap_t *map)
{
	...
	map->node_list = vzalloc(map->node_count * sizeof(policy_node_t));
	if (!map->node_list) {
...
static int pp_read_perm(struct policy_file *fp, policy_perm_t *perm)
{
	...
	perm->name = vzalloc(perm->name_len + 1);
	if (!perm->name) {
		pr_err("SELinux: unable to allocate memory for policydb "
					"string of length %d\n", perm->name_len + 1);
		return -ENOMEM;
...
static int pp_read_cons(struct policy_file *fp, policy_constraints_t *cons)
{
	...
	cons->expression_list = vzalloc(cons->expression_num * sizeof(policy_expression_t));
...
static int pp_read_common_helper(struct policy_file *fp, policy_common_t *common)
{
	...
	if (common->name_len) {
		common->name = vzalloc(common->name_len + 1);
		if (!common->name) {
			pr_err("SELinux: unable to allocate memory for policydb "
						"string of length %d\n", common->name_len + 1);
			return -ENOMEM;
	...
	if (common->element_num) {
		common->perm_list = vzalloc(common->element_num * sizeof(policy_perm_t));
		if (!common->perm_list) {
			pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_read_common(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_common.attr_list = vzalloc(db->symtab_common.element_num * sizeof(policy_common_t));
...
pp_read_common_fail:
	...
	if (db->symtab_common.attr_list) {
		vfree(db->symtab_common.attr_list);
...
static int pp_read_classes_helper(struct policy_file *fp, policy_classes_t *classes)
{
	...
	if (classes->name_len) {
		classes->name = vzalloc(classes->name_len + 1);
		if (!classes->name) {
			pr_err("SELinux: unable to allocate memory for policydb "
						"string of length %d\n", classes->name_len + 1);
			return -ENOMEM;
	...
	if (classes->common_name_len) {
		classes->common_name = vzalloc(classes->common_name_len + 1);
	...
	if (classes->element_num) {
		classes->perm_list = vzalloc(classes->element_num * sizeof(policy_perm_t));
	...
	if (classes->constraints_num) {
		classes->constraints_list = vzalloc(classes->constraints_num * sizeof(policy_constraints_t));
	...
	if (classes->validatetrans_num) {
		classes->validatetrans_list = vzalloc(classes->validatetrans_num * sizeof(policy_constraints_t));
...
pp_read_classes_helper_fail:
	...
	if (classes->validatetrans_list) {
		vfree(classes->validatetrans_list);
		classes->validatetrans_list = NULL;
	}

	if (classes->constraints_list) {
		vfree(classes->constraints_list);
		classes->constraints_list = NULL;
	}

	if (classes->perm_list) {
		vfree(classes->perm_list);
		classes->perm_list = NULL;
	}

	if (classes->common_name) {
		vfree(classes->common_name);
...
static int pp_read_classes(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_classes.attr_list = vzalloc(db->symtab_classes.element_num * sizeof(policy_classes_t));
...
pp_read_classes_fail:
	...
	if (db->symtab_classes.attr_list) {
		vfree(db->symtab_classes.attr_list);
...
static int pp_read_roles_helper(struct policy_file *fp, policy_roles_t *roles)
{
	...
	if (roles->name_len) {
	roles->name = vzalloc(roles->name_len + 1);
		if (!roles->name) {
			pr_err("SELinux: unable to allocate memory for policydb "
						"string of length %d\n", roles->name_len + 1);
			return -ENOMEM;
...
static int pp_read_roles(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_roles.attr_list = vzalloc(db->symtab_roles.element_num * sizeof(policy_roles_t));
...
pp_read_roles_fail:
	...
	if (db->symtab_roles.attr_list) {
		vfree(db->symtab_roles.attr_list);
...
static int pp_read_types_helper(struct policy_file *fp, policy_types_t *types)
{
	...
	types->name = vzalloc(types->name_len + 1);
	if (!types->name) {
		pr_err("SELinux: unable to allocate memory for policydb "
					"string of length %d\n", types->name_len + 1);
		return -ENOMEM;
...
static int pp_read_types(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_types.attr_list = vzalloc(db->symtab_types.element_num * sizeof(policy_types_t));
...
pp_read_types_fail:
	...
	if (db->symtab_types.attr_list) {
		vfree(db->symtab_types.attr_list);
...
static int pp_read_users_helper(struct policy_file *fp, policy_users_t *users)
{
	...
	if (users->name_len) {
		users->name = vzalloc(users->name_len + 1);
		if (!users->name) {
			pr_err("SELinux: unable to allocate memory for policydb "
						"string of length %d\n", users->name_len + 1);
			return -ENOMEM;
...
static int pp_read_users(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_users.attr_list = vzalloc(db->symtab_users.element_num * sizeof(policy_users_t));
...
pp_read_users_fail:
	...
	if (db->symtab_users.attr_list) {
		vfree(db->symtab_users.attr_list);
...
static int pp_read_bools_helper(struct policy_file *fp, policy_bools_t *bools)
{
	...
	bools->name = vzalloc(bools->name_len + 1);
	if (!bools->name) {
		pr_err("SELinux: unable to allocate memory for policydb "
					"string of length %d\n", bools->name_len + 1);
		return -ENOMEM;
...
static int pp_read_bools(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_bools.attr_list = vzalloc(db->symtab_bools.element_num * sizeof(policy_bools_t));
...
pp_read_bools_fail:
	...
	if (db->symtab_bools.attr_list) {
		vfree(db->symtab_bools.attr_list);
...
static int pp_read_levels_helper(struct policy_file *fp, policy_levels_t *levels)
{
	...
	if (levels->name_len) {
		levels->name = vzalloc(levels->name_len + 1);
		if (!levels->name) {
			pr_err("SELinux: unable to allocate memory for policydb "
						"string of length %d\n", levels->name_len + 1);
			return -ENOMEM;
...
static int pp_read_levels(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_levels.attr_list = vzalloc(db->symtab_levels.element_num * sizeof(policy_levels_t));
...
pp_read_levels_fail:
	...
	if (db->symtab_levels.attr_list) {
		vfree(db->symtab_levels.attr_list);
...
static int pp_read_cats_helper(struct policy_file *fp, policy_cats_t *cats)
{
	...
	cats->name = vzalloc(cats->name_len + 1);
	if (!cats->name) {
		pr_err("SELinux: unable to allocate memory for policydb "
					"string of length %d\n", cats->name_len + 1);
		return -ENOMEM;
...
static int pp_read_cats(struct policy_file *fp, policy_db_t *db)
{
	...
	db->symtab_cats.attr_list = vzalloc(db->symtab_cats.element_num * sizeof(policy_cats_t));
...
pp_read_cats_fail:
	...
	if (db->symtab_cats.attr_list) {
		vfree(db->symtab_cats.attr_list);
...
static int pp_read_condlist_item(struct policy_file *fp, policy_condlist_item_t *item)
{
	...
	if (item->expression_num) {
		item->expression_list = vzalloc(item->expression_num * sizeof(policy_cond_expr_t));
	...
	if (item->true_list.element_num) {
		item->true_list.av_list = vzalloc(item->true_list.element_num * sizeof(policy_avtab_item_t));
	...
	if (item->false_list.element_num) {
		item->false_list.av_list = vzalloc(item->false_list.element_num * sizeof(policy_avtab_item_t));
...
pp_read_condlist_item_fail:
	...
	if (item->false_list.av_list) {
		vfree(item->false_list.av_list);
		item->false_list.av_list = NULL;
	}

	if (item->true_list.av_list) {
		vfree(item->true_list.av_list);
		item->true_list.av_list = NULL;
	}

	if (item->expression_list) {
		vfree(item->expression_list);
...
static int pp_read_filenametrans_item(struct policy_file *fp, policy_filenametrans_item_t *item)
{
	...
	if (item->name_len) {
		item->name = vzalloc(item->name_len + 1);
		if (!item->name) {
			pr_err("SELinux: unable to allocate memory for policydb "
						"string of length %d\n", item->name_len + 1);
			return -ENOMEM;
...
static int pp_read_ocon_isid(struct policy_file *fp, policy_ocontext_t *ocon)
{
	...
	ocon->isid_list = vzalloc(ocon->isid_num * sizeof(policy_ocon_isid_t));
	if (!ocon->isid_list) {
		pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_read_ocon_fs(struct policy_file *fp, policy_ocontext_t *ocon)
{
	...
	ocon->fs_list = vzalloc(ocon->fs_num * sizeof(policy_ocon_fs_t));
	if (!ocon->fs_list) {
		pr_err("SELinux: unable to allocate memory for policydb "
	...
	for (i = 0; i < ocon->fs_num; ++i) {
		...
		if (list[i].name_len) {
			list[i].name = vzalloc(list[i].name_len + 1);
			if (!list[i].name) {
				pr_err("SELinux: unable to allocate memory for policydb "
							"string of length %d\n", list[i].name_len + 1);
				goto pp_read_ocon_fs_fail;
...
static int pp_read_ocon_port(struct policy_file *fp, policy_ocontext_t *ocon)
{
	...
	ocon->port_list = vzalloc(ocon->port_num * sizeof(policy_ocon_port_t));
	if (!ocon->port_list) {
		pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_read_ocon_netif(struct policy_file *fp, policy_ocontext_t *ocon)
{
	...
	ocon->netif_list = vzalloc(ocon->netif_num * sizeof(policy_ocon_netif_t));
	if (!ocon->netif_list) {
		pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_read_ocon_node(struct policy_file *fp, policy_ocontext_t *ocon)
{
	...
	ocon->node_list = vzalloc(ocon->node_num * sizeof(policy_ocon_node_t));
	if (!ocon->node_list) {
		pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_read_ocon_fsuse(struct policy_file *fp, policy_ocontext_t *ocon)
{
	...
	ocon->fsuse_list = vzalloc(ocon->fsuse_num * sizeof(policy_ocon_fsuse_t));
	if (!ocon->fsuse_list) {
		pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_read_ocon_node6(struct policy_file *fp, policy_ocontext_t *ocon)
{
	...
	ocon->node6_list = vzalloc(ocon->node6_num * sizeof(policy_ocon_node6_t));
	if (!ocon->node_list) {
		pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_read_genfs_item(struct policy_file *fp, policy_genfs_item_t *item)
{
	...
	if (item->fstype_len) {
		item->fstype = vzalloc(item->fstype_len + 1);
		if (!item->fstype) {
			pr_err("SELinux: unable to allocate memory for policydb "
						"string of length %d\n", item->fstype_len + 1);
			return -ENOMEM;
	...
	if (item->ocon_num) {
		item->ocon_list = vzalloc(item->ocon_num * sizeof(policy_genfs_ocon_item_t));
		if (!item->ocon_list) {
			pr_err("SELinux: unable to allocate memory for policydb "
...
static int pp_parse_header(struct policy_file *fp, policy_db_t *db)
{
	...
	if (db->target_str_len) {
		db->target_str = vzalloc(db->target_str_len + 1);
		if (!db->target_str) {
			pr_err("SELinux: unable to allocate memory for policydb "
			       "string of length %d\n", db->target_str_len + 1);
			return -ENOMEM;
...
static int pp_parse_avtab(struct policy_file *fp, policy_db_t *db)
{
	...
	db->avtab.attr_list = vzalloc(db->avtab.element_num * sizeof(policy_avtab_item_t));
...
static int pp_parse_condlist(struct policy_file *fp, policy_db_t *db)
{
	...
	db->condlist.attr_list = vzalloc(db->condlist.element_num * sizeof(policy_condlist_item_t));
...
static int pp_parse_roletrans(struct policy_file *fp, policy_db_t *db)
{
	...
	db->roletrans.attr_list = vzalloc(db->roletrans.element_num * sizeof(policy_roletrans_item_t));
...
static int pp_parse_roleallow(struct policy_file *fp, policy_db_t *db)
{
	...
	db->roleallow.attr_list = vzalloc(db->roleallow.element_num * sizeof(policy_roleallow_item_t));
...
static int pp_parse_filenametrans(struct policy_file *fp, policy_db_t *db)
{
	...
	db->filenametrans.attr_list = vzalloc(db->filenametrans.element_num * sizeof(policy_filenametrans_item_t));
...
pp_parse_filenametrans_fail:
	...
	if (db->filenametrans.attr_list) {
		vfree(db->filenametrans.attr_list);
...
static int pp_parse_genfs(struct policy_file *fp, policy_db_t *db)
{
	...
	db->genfs.attr_list = vzalloc(db->genfs.element_num * sizeof(policy_genfs_item_t));
...
static int pp_parse_range(struct policy_file *fp, policy_db_t *db)
{
	...
	db->range.attr_list = vzalloc(db->range.element_num * sizeof(policy_range_item_t));
...
static int pp_parse_typeattr_map(struct policy_file *fp, policy_db_t *db)
{
	...
	db->typeattr_map.attr_list = vzalloc(db->typeattr_map.element_num * sizeof(policy_typeattr_map_item_t));
...
pp_parse_typeattr_map_fail:
	...
	if (db->typeattr_map.attr_list) {
		vfree(db->typeattr_map.attr_list);
...
static int pp_new_permissive(policy_db_t *db, pp_perm_desc_t *desc, size_t *len)
{
	...
	node_count++;
	ptr = vzalloc(node_count * sizeof(policy_node_t));
	if (!ptr) {
...
static int pp_new_avc(policy_db_t *db, pp_avc_desc_t *desc, size_t *len)
{
	...
	num++;
	ptr = vzalloc(num * sizeof(policy_avtab_item_t));
	if (!ptr) {
		pr_err("SELinux: unable to reallocate memory for policydb "
	...
	list[num - 1].datums = pp_name_to_perm(db, desc, desc->perm);
...
{
	policy_avtab_item_t *list = (policy_avtab_item_t *)db->avtab.attr_list;
	uint32_t av = pp_name_to_perm(db, desc, desc->perm);
...
static int pp_preproc_permissive(policy_db_t *db, pp_perm_desc_t *list, size_t list_len, size_t *perm_len)
{
	...
	for (i = 0; i < list_len; ++i) {
		...
		if (list[i].op & OP_NEW) {
			rc = pp_new_permissive(db, &list[i], perm_len);
			if (rc) {
				break;
			}
		} else {
...
static int pp_preproc_avc(policy_db_t *db, pp_avc_desc_t *list, size_t list_len, size_t *avc_len)
{
	...
	for (i = 0; i < list_len; ++i) {
		if (pp_find_avc(db, &list[i], &index)) {
			if (list[i].op & OP_MOD) {
				rc = pp_mod_avc(db, index, &list[i], avc_len);
				if (rc) {
					break;
				}
			}
			...
			if (list[i].op & OP_NEW) {
				rc = pp_new_avc(db, &list[i], avc_len);
				if (rc) {
					break;
				}
			}
...
static int pp_postproc_permissive(policy_db_t *db, pp_perm_desc_t *list, size_t list_len, size_t *perm_len)
{
	...
	for (i = 0; i < list_len; ++i) {
		...
		if (list[i].op & OP_DEL) {
			rc = pp_del_permissive(db, index, &list[i], perm_len);
			if (rc) {
				break;
			}
		} else {
...
static int pp_postproc_avc(policy_db_t *db, pp_avc_desc_t *list, size_t list_len, size_t *avc_len)
{
	...
	for (i = 0; i < list_len; ++i) {
		...
		if (list[i].op & OP_DEL) {
			rc = pp_del_avc(db, index, &list[i], avc_len);
			if (rc) {
				break;
			}
		} else if (list[i].op & OP_UNMOD) {
			rc = pp_unmod_avc(db, index, &list[i], avc_len);
			if (rc) {
				break;
			}
		} else {
...
{
	pp_avc_desc_t *list = &pp_avc_desc_list[SECURITY_SETENFORCE];
	size_t len;
	...
	perm = string_to_av_perm(pol, class, list->perm);
	*perms &= ~perm;
